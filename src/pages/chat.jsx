...
import { 
  ...
  Mic, Image as ImageIcon, X, Share2, Trash2
} from "lucide-react";
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        if (audioBlob) {
            ...
            finalText = data.transcript || "(Audio Message)"; // Fallback
      ...
      `}>
        ...
        <div className="relative w-full max-w-3xl mx-auto">
          ...
          <p className={`text-center text-[10px] text-neutral-600 mt-4 font-mono transition-opacity duration-500 ${isChatStarted ? "opacity-100" : "opacity-0"}`}>
             Gemini 2.5 Flash // {isRecording ? <span className="text-red-500 font-bold">RECORDING...</span> : "Multi-Agent"}
          </p>
        </div>
      </div>
    </div>
