...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    let finalText = input;
    let userMsgObject = { role: "user", content: input, imagePreview };
    ...
    try {
      if (audioBlob) {
        ...
        userMsgObject = { ...userMsgObject, type: "voice", audioUrl: audioPreviewUrl, content: finalText };
      }
      ...
      const response = await getAgentResponse(finalText, selectedAgent, newMessages.slice(-5), null, imageBase64);
    ...
    <div className="flex-1 flex flex-col h-full relative w-full overflow-hidden">
          ...
          <div className="space-y-10 max-w-3xl mx-auto">
            {messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"} gap-2`}>
                ...
                >
                  {m.imagePreview && <img src={m.imagePreview} className="mb-3 rounded-xl max-w-xs border border-neutral-800" alt="Upload" />}
                  {m.type === "voice" && <div className="mb-2"><VoiceMessage src={m.audioUrl} /></div>}
      ...
      >
        ...
        <div className="w-full max-w-3xl pointer-events-auto">
          <ChatInputBox 
            {...{input, setInput, textareaRef, isRecording, startRecording, stopRecording, audioPreviewUrl, clearAudio, imagePreview, setImagePreview, handleImageUpload, fileInputRef, handleSend, loading, agents, selectedAgent, setSelectedAgent, showAgentMenu, setShowAgentMenu, setMessages}} 
          />
        </div>
      </div>
    </div>
